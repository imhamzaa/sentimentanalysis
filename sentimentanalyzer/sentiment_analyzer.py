...
class SentimentAnalyzer:
    ...
    def apply_sgd_algo(self, X_train_tfidf, ytrain):
        clf = SGDClassifier(loss='hinge', penalty='l2', alpha=1e-3, random_state=42).fit(X_train_tfidf, ytrain)
        return clf

    def predict_multinomial_result(self, clf, count_vect, tfidf_transformer, input_value):
        X_new_counts = count_vect.transform([input_value])
        X_new_tfidf = tfidf_transformer.transform(X_new_counts)
        predicted_result = clf.predict(X_new_tfidf)
        return predicted_result

    def predict_sgd_result(self, clf, count_vect, tfidf_transformer, input_value):
        X_new_counts = count_vect.transform([input_value])
        X_new_tfidf = tfidf_transformer.transform(X_new_counts)
        predicted_result = clf.predict(X_new_tfidf)
        return predicted_result
...
sgd_algo = sentiment_analyzer.apply_sgd_algo(X_train_tfidf, ytrain)
...
multinomial_result = sentiment_analyzer.predict_multinomial_result(multinomial_algo, count_vect, tfidf_transformer,
                                                                   input_value)
sgd_result = sentiment_analyzer.predict_sgd_result(sgd_algo, count_vect, tfidf_transformer, input_value)
...
if predicted_result_accuracy_mul > predicted_result_accuracy_sgd:
    ...
    print(f'\nAccuracy of prediction is {predicted_result_accuracy_mul:.2f}%')
elif predicted_result_accuracy_mul < predicted_result_accuracy_sgd:
    print('\n************** SGD Algorithm **************')
    print(f'\nPredicted result of "{input_text}" is "{sgd_result[0]}"')
    print(f'\nAccuracy of prediction is {predicted_result_accuracy_sgd:.2f}%')
else:
    print('\n********** Multinomial Algorithm **********')
    print(f'\nPredicted result of "{input_text}" is "{multinomial_result[0]}"')
    print(f'\nAccuracy of prediction is {predicted_result_accuracy_mul:.2f}%')
